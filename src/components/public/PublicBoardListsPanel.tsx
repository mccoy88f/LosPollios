...
export function PublicBoardListsPanel({
  ...
  if (!lists.length) {
    return (
      <p className="flex-1 flex items-center justify-center text-slate-400 text-xs">Nessuna lista</p>
    )
  }
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-1 p-1 sm:p-1.5 overflow-hidden">
      ...
      >
        {layout.mode === 'coalitions' ? (
          layout.heroCoalitions.map(c => (
            <CoalitionHero key={c.name} block={c} totalVoters={totalVoters} />
          ))
        ) : (
      ...
      {restRows.length > 0 && (
        <div className="flex-[2] min-h-0 flex flex-col overflow-hidden border-t border-slate-100 pt-1">
          <p className="shrink-0 text-[clamp(0.5rem,1.6vw,0.625rem)] text-slate-400 uppercase tracking-wide px-1 mb-0.5">
...
function CompactListRow({
    ...
    return (
      <div className="flex-1 min-h-0 flex items-center gap-1.5 px-1.5 overflow-hidden">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[clamp(0.5rem,1.6vw,0.625rem)] text-slate-400">{listCount} liste</p>
        </div>
        <div className="shrink-0 text-right tabular-nums">
          <p className="font-semibold text-[clamp(0.6rem,2vw,0.8125rem)]">
            {coalition.votes.toLocaleString('it-IT')}
          </p>
          <p className="text-slate-500 text-[clamp(0.5rem,1.6vw,0.625rem)]">
            {coalition.pctOnVoters.toFixed(1)}%
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex-1 min-h-0 flex items-center gap-1.5 px-1.5 overflow-hidden">
      ...
      <div className="flex-1 min-w-0">
        <p className="truncate text-[clamp(0.55rem,1.9vw,0.75rem)] font-medium">{list.shortName || list.name}</p>
        <div className="h-0.5 sm:h-1 bg-slate-100 rounded-full mt-0.5 overflow-hidden">
          <div
      ...
      <div className="shrink-0 text-right tabular-nums leading-tight">
        ...
        <p className="text-slate-500 text-[clamp(0.5rem,1.6vw,0.625rem)]">{pct.toFixed(1)}%</p>
      </div>
    </div>
  )
}

function CoalitionHero({ block, totalVoters }: { block: PublicBoardCoalitionBlock; totalVoters: number }) {
  return (
    <div className="min-h-0 flex flex-col rounded-lg bg-slate-50 border border-slate-100 p-1.5 sm:p-2 overflow-hidden">
      <div className="shrink-0 flex items-start gap-2 min-w-0 mb-1">
        ...
        <div className="flex-1 min-w-0">
          ...
          {block.mayorLabel && (
            <p className="text-slate-500 truncate text-[clamp(0.55rem,2vw,0.75rem)]">{block.mayorLabel}</p>
          )}
          <p className="tabular-nums font-bold text-[clamp(0.8rem,3.5vw,1.25rem)] mt-0.5">
            {block.votes.toLocaleString('it-IT')}
            <span className="font-normal text-slate-500 text-[clamp(0.6rem,2.2vw,0.875rem)]">
              {' '}
              / {totalVoters > 0 ? totalVoters.toLocaleString('it-IT') : '—'}
            </span>
          </p>
          <p className="text-[clamp(0.6rem,2.2vw,0.875rem)] font-medium text-slate-600">
            {block.pctOnVoters.toFixed(1)}% su votanti
          </p>
        </div>
      </div>
      <div className="h-1.5 bg-slate-200 rounded-full shrink-0 overflow-hidden mb-1">
        <div
          className="h-full rounded-full transition-all duration-500"
          style={{ width: `${Math.min(100, block.pctOnVoters)}%`, backgroundColor: block.color }}
        />
      </div>
...
function ListHero({
  ...
  if (nested) {
    return (
      <div className="flex items-center gap-2 min-h-0 bg-white/80 rounded-md px-1.5 py-1 border border-slate-100">
        ...
        <div className="flex-1 min-w-0">
          ...
          {list.candidateMayor && (
            <p className="text-slate-500 truncate text-[clamp(0.5rem,1.8vw,0.6875rem)]">{list.candidateMayor}</p>
          )}
        </div>
        <div className="shrink-0 text-right tabular-nums">
          <p className="font-bold text-[clamp(0.65rem,2.5vw,0.9375rem)]">{list.votes.toLocaleString('it-IT')}</p>
          <p className="text-slate-500 text-[clamp(0.5rem,1.8vw,0.6875rem)]">{pct.toFixed(1)}%</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-0 flex flex-col justify-center rounded-lg bg-slate-50 border border-slate-100 p-2 sm:p-2.5 overflow-hidden">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          {(list.candidateMayor || list.coalition) && (
            <p className="text-slate-500 truncate text-[clamp(0.55rem,2vw,0.75rem)]">
              {list.candidateMayor}
              {list.candidateMayor && list.coalition ? ' · ' : ''}
              {list.coalition}
            </p>
        ...
        <div className="shrink-0 text-right tabular-nums">
          <p className="font-bold text-[clamp(0.85rem,3.8vw,1.375rem)]">
            ...
            <span className="font-normal text-slate-500 text-[clamp(0.55rem,2vw,0.75rem)] block sm:inline">
              {' '}
              / {totalVoters > 0 ? totalVoters.toLocaleString('it-IT') : '—'}
            </span>
          </p>
          <p className="font-medium text-slate-600 text-[clamp(0.6rem,2.2vw,0.875rem)]">{pct.toFixed(1)}%</p>
        </div>
      </div>
      <div className="h-2 sm:h-2.5 bg-slate-200 rounded-full mt-2 overflow-hidden shrink-0">
        <div
          className="h-full rounded-full transition-all duration-500"
          style={{ width: `${Math.min(100, pct)}%`, backgroundColor: list.color }}
        />
  ...
  if (list.listLogoUrl) {
    return (
      ...
      <img
        ...
        alt=""
        className={cn(cls, 'object-contain rounded-md shrink-0 bg-white border border-slate-100')}
      />
    )
  }
  return (
