...
function ElectionCard({
  ...
  e: {
    ...
    status: string
    archived: boolean
    _count: { sections: number; lists: number }
  }
}) {
  return (
...
export default async function AdminPage() {
  const [activeElections, archivedElections] = await Promise.all([
    prisma.election.findMany({
      where: { archived: false },
      orderBy: { date: 'desc' },
      include: { _count: { select: { sections: true, lists: true } } },
    }),
    prisma.election.findMany({
      where: { archived: true },
      orderBy: { date: 'desc' },
  ...
  const empty = activeElections.length === 0 && archivedElections.length === 0
      ...
      {empty ? (
        ...
        <div className="space-y-10">
          {activeElections.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">In corso e pianificate</h2>
          ...
          {archivedElections.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Archivio</h2>
              <div className="grid gap-4">
